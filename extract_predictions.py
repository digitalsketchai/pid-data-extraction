...
def get_all_predictions(base_url: str, api_key: str, imageid: str) -> dict:
    url = f"{base_url}/digitalsketch/diagram/predictions/all"
    print(f"  POST {url}")
    resp = requests.post(url, json={"api_key": api_key, "imageid": imageid}, timeout=120)
    resp.raise_for_status()
    return resp.json()
...
def process_image(base_url: str, api_key: str, imageid: str, page_num: int, total_pages: int) -> dict:
    ...
    if image_size and image_size < 100:
        ...
        print(f"  Returning empty predictions for this page.")
        return {
            "success": False,
            "imageid": imageid,
            "predictions": [],
    ...
    print(f"{label} 7. GET /digitalsketch/diagram/predictions/all")
    return get_all_predictions(base_url, api_key, imageid)
...
def main() -> int:
    ...
    for idx, imageid in enumerate(imageids, start=1):
        ...
        predictions = process_image(base_url, api_key, imageid, idx, total_pages)

        if is_pdf:
            out_path = OUTPUT_DIR / f"{sample_path.stem}_page{idx}_{imageid}_predictions.json"
        else:
            out_path = OUTPUT_DIR / f"{sample_path.stem}_{imageid}_predictions.json"
